        ...
        if os.path.isfile(self.serviceCSRFile):
            ...
            try:
                ...
                if res.status_code != requests.codes.ok:
                    ...
                    self.logger.info("Saving service certificate.")
...
class CorrelationToken():
    ...
    def __init__(self, logger, debug=False):
        self.logger = logger
        self.debug = debug
        self.certFileName = 'cert.pem' # CA certificate
        if self.debug:
            self.getCertRemote()
    ...
    def getCertRemote(self):
        ...
        if res.status_code != requests.codes.ok:
            ...
            with open(self.certFileName,'w') as f:
                f.write(res.json()['PEM'])
                self.logger.info("Certificate saved.")

    def getPubKey(self):
        """Retrieve a public key from the full certificate."""
        if os.path.isfile(self.certFileName):
            with open(self.certFileName,'r') as f:
                certPEM = f.read()
    ...
    def getTestToken(self):
        ...
        if self.debug:
            ...
            try:
                ...
                self.logger.error("Token service is unavailable.")
                return
            ...
            if res.status_code != requests.codes.ok:
                ...
                if 'access_token' in data_json:
                    ...
                    self.logger.warning("No access token found, resp: %s" \
                                        % data_json)
        else:
            msg = "No unsafe functionality is allowed. Do better"
            self.logger.warning(msg)
...
def main():
    ...
    import os
    serviceName = os.path.basename(os.path.dirname(os.path.realpath(__file__)))
    logger = log(serviceName).logger

    ServiceCert(logger, serviceName, debug=True)
    CorrelationToken(logger, debug=True)
